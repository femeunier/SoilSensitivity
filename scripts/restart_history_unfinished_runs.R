  ...
  for(iy in seq(1,length(Y))){
    ...
    if (land[which(rownames(land)==clon),which(colnames(land)==clat)] != 0){
      ...
      for (iscenar in seq(1,length(scenars))){
        ...
        run_origin <- file.path(rundir,run_origin)
        out_origin <- file.path(outdir,run_origin)
        ...
        if (status %in% c("Started","Error")){ # Change ED2IN and restart
          ...
          fyear <- sort(as.numeric(sapply(str_split(basename(rownames(files.OP.ordered)),pattern = "-"),"[[",c(3))),decreasing = TRUE)[1]
          ...
          if (nrow(details.file)>0){
            ...
            ed2in$SFILIN <- file.path(out_origin,"histo","history")
          ...
          # job.sh
          write_joblauncher_noR_status(file =  file.path(dir_joblauncher,"job_historical.sh"),
...
dumb <- write_bash_submission(file = file.path(rundir,"all_jobs_restart_historical.sh"),
                              list_files = list_dir,
                              job_name = "job_historical.sh")

# scp /home/femeunier/Documents/projects/SoilSensitivity/scripts/restart_history_unfinished_runs.R hpc:/data/gent/vo/000/gvo00074/felicien/R
